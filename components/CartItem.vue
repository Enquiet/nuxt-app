<template>
  <li class="product-cart__item">
    <img class="product-cart__img" src="@/assets/image/product.png"/>
    <div class="product-cart__wrapper">
      <p class="product-cart__title">{{product.name}}</p>
      <p class="product-cart__price">{{product.price}}₽</p>
      <div class="product-cart__inner raiting">
        ...
        <path class="raiting__progress" d="M6.5 11L4 8H10H16L13.5 11V14.5L13 15.5L10 14L9 14.5L6.5 15.5V14.5V11Z " fill="#F2C94C"/>
      </svg>
        <span class="product-cart__rating raiting-status">{{product.rating}}</span>
      </div>
    </div>
    <button class="product-cart__btn" @click.prevent="deleteProduct(product.id)">
      <svg class="product-cart__btn--img" width="20" height="22" viewBox="0 0 20 22" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  methods:{
    ...mapMutations({deleteProductCart: 'cart/deleteProductCart'}),
    deleteProduct(id){
